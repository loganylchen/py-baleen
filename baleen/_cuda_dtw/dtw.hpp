                            ...
                            const int sakoe_band)
{
            ...
            i - threadIdx.x < first_seq_length)
        {
            ...
            if (diag_cost > up_cost)
            {
                if (up_cost > right_cost)
                {
                    ...
                    if (pathMatrix != 0)
                    {
                        pathMatrix[pitchedCoord((newDtwCostSoFar ? offset_within_second_seq : 0) + threadIdx.x, i - threadIdx.x, pathMemPitch)] = used_open_right_end_cost ? OPEN_RIGHT : RIGHT;
                    }
                    // move = 'R';
                }
                else
                {
                    ...
                    if (pathMatrix != 0)
                    {
                        pathMatrix[pitchedCoord((newDtwCostSoFar ? offset_within_second_seq : 0) + threadIdx.x, i - threadIdx.x, pathMemPitch)] = UP;
                    }
                    // move = 'U';
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    if (pathMatrix != 0)
                    {
                        pathMatrix[pitchedCoord((newDtwCostSoFar ? offset_within_second_seq : 0) + threadIdx.x, i - threadIdx.x, pathMemPitch)] = DIAGONAL;
                    }
                    // move = 'D';
                }
            }
    ...
    if (offset_within_second_seq + blockDim.x >= second_seq_length)
    {
        if (dtwPairwiseDistances != 0 && threadIdx.x == 0 && newDtwCostSoFar != 0)
        {
            ...
            { // use the distance as-is (similar length sequences will tend to cluster together)
