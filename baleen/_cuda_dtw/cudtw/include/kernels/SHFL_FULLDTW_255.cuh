...
// 8 values per thread no shared memory
template <
    typename index_t,
    typename value_t> __global__
void shfl_FullDTW_255( // was DTW_fast_256_shuffle_kernel_no_shared_memory
    const value_t * Query,
    value_t * Subject,
    value_t * Dist,
    index_t num_entries,
    ...
    //for (index_t l = thid; l < lane/8; l += blockDim.x) {
    //    const index_t iter = l/WARP_SIZE;
        ...
        const value_t subject_value0 = l == 0 ? 0 : Subject[base+8*l-1];
        ...
        for (index_t k = 3; k < lane+WARP_SIZE-1; k++) {
            ...
            //if (iter < lane/WARP_SIZE-1 && thid == 31 && k>l) Subject_cache[i] = penalty_here7;
        ...
        penalty_here6 = (query_value-subject_value6) * (query_value-subject_value6) + min(penalty_here5, min(penalty_here6, penalty_temp1));
        penalty_here7 = (query_value-subject_value7) * (query_value-subject_value7) + min(penalty_here6, min(penalty_here7, penalty_temp0));
        //if (thid == 31 && iter < lane/WARP_SIZE-1) Subject_cache[lane+(iter+1)*WARP_SIZE-1-l] = penalty_here7;
        //iter++;
